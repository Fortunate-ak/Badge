...
export default function DocumentCard({ value }: { value: Document }) {
    ...
    return <div className="flex flex-col p-4 border border-border rounded-md bg-secondary group transition-all cursor-pointer">
        ...
        {value.updated_at && <span className="text-xs mb-2 text-muted">Uploaded on {new Date(value.updated_at).toLocaleDateString()}</span>}
        <div className="flex flex-row gap-1 -mb-1">
            {
                (value.document_categories_details || []).map((v, k) => {
                    return <span key={k+"-doc-category"} className="tw-tag text-xs first-letter:uppercase lowercase">{v.name}</span>
                })
            }
        </div>
    </div>
