...
export default function Opportunities() {
    ...
    return <div>
        <div className="tw-dashboard-grid">
            {ops.map((opportunity, index) => <OpportunityCard id={opportunity.id} key={index} expiry_date={opportunity.expiry_date} title={opportunity.title} description={opportunity.description} tags={opportunity.tags} company={opportunity.institution_details?.name || ""} logo={opportunity.institution_details?.profile_image?.replace("localhost", window.location.host)}/>)}
        </div>
    </div>
}